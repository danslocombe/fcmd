...
test "concurrent_reads" {
    // Inserts strings sequentially, then reads them concurrently from 4 processes.
    // Tests that multiple processes can simultaneously read from the same mmap —
    // the primary cross-process use case. Concurrent writes are not safe without a
    // cross-process mutex (DumbList.append uses non-atomic load+store on len), so
    // this test validates reads only.
    const temp_dir = (std.process.Environ{ .block = .global }).getAlloc(std.testing.allocator, "TEMP") catch (std.process.Environ{ .block = .global }).getAlloc(std.testing.allocator, "TMP") catch unreachable;
    defer std.testing.allocator.free(temp_dir);
    ...
    // Insert all strings sequentially so the mmap is in a known good state.
    for (strings) |str| {
        var ctrl = test_mp.ProcessController.init(std.testing.allocator);
        defer ctrl.deinit();
        const args = [_][]const u8{ exe_path, "--test-mp", "insert", test_state_path, str };
