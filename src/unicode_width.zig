...
// Adapted from
// https://github.com/unicode-rs/unicode-width/blob/master/src/tables.rs
...
pub fn get_width_inner(cp: u32) usize {
    if (cp < '\x7F') {
        if (cp >= '\x20') {
...
fn lookup_width(p_cp: u32) usize {
    ...
    const render_ambiguous_characters_as_doublewidth = false;

    // A width of 3 signifies that the codepoint is ambiguous width.
    if (width == 3) {
        if (render_ambiguous_characters_as_doublewidth) {
            return 2;
        } else {
            return 1;
        }
...
test "blah frog" {
    try std.testing.expectEqual(@as(usize, 2), get_width_slice("🐸"));
